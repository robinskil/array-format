...
async fn main() {
    ...
    file.write_array("signal", vec![0], first.view()).await.unwrap();

    // Partial write into chunk 1: only indices 5-6
    let patch = Array::from_vec(vec![99i32, 100]).into_dyn();
    file.write_array("signal", vec![5], patch.view()).await.unwrap();
    ...
    let full = file.read_array::<i32>("signal", vec![], vec![]).await.unwrap();
    ...
    let sub = file.read_array::<i32>("signal", vec![4], vec![5]).await.unwrap();
