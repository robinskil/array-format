...
async fn main() {
    ...
    file.set_attribute("pressure", "scale_factor", AttributeValue::Float64(0.01)).unwrap();
    file.set_attribute("pressure", "valid_min", AttributeValue::Float32(0.0)).unwrap();

    let units = file.get_attribute("pressure", "units").unwrap().unwrap();
    let scale = file.get_attribute("pressure", "scale_factor").unwrap().unwrap();
