...
mod tests {
    ...
    use crate::v6::{AttributeValue, StringPool};
    ...
    fn chunk_lookup_uses_the_sorted_order() {
        let meta = &sample_footer().arrays[0];
        assert_eq!(meta.chunk(&[1, 0]).map(|a| a.offset), Some(8000));
        assert_eq!(meta.chunk(&[0, 0]).map(|a| a.offset), Some(0));
        assert!(meta.chunk(&[2, 0]).is_none());
        assert!(meta.chunk(&[0, 1]).is_none());
    }
