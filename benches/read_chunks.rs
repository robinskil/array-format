...
fn bench_read_all_chunks(c: &mut Criterion) {
    ...
    let file_no_pat = rt.block_on(prepare_file(NoCompression, &patterned));
    ...
    let file_zstd_pat = rt.block_on(prepare_file(ZstdCodec::default(), &patterned));
    ...
    let file_lz4_pat = rt.block_on(prepare_file(Lz4Codec, &patterned));
    ...
    let file_no_rnd = rt.block_on(prepare_file(NoCompression, &random));
    ...
    let file_zstd_rnd = rt.block_on(prepare_file(ZstdCodec::default(), &random));
    ...
    let file_lz4_rnd = rt.block_on(prepare_file(Lz4Codec, &random));
...
fn bench_single_chunk_read(c: &mut Criterion) {
    ...
    let file_no = rt.block_on(prepare_file(NoCompression, &patterned));
    rt.block_on(async {
        file_no
    ...
    let file_zstd = rt.block_on(prepare_file(ZstdCodec::default(), &patterned));
    rt.block_on(async {
        file_zstd
            .read_array::<u8>("data", vec![0, 0], vec![1, CHUNK_SIZE])
            .await
    ...
    let file_lz4 = rt.block_on(prepare_file(Lz4Codec, &patterned));
    rt.block_on(async {
        file_lz4
            .read_array::<u8>("data", vec![0, 0], vec![1, CHUNK_SIZE])
            .await
